...
# MPFB parents the basemesh to the armature; scale only the parent to avoid s^2.
rig.scale=(s,s,s)
a,b=zb(body); rig.location.z-=a
bpy.context.view_layer.update(); a,b=zb(body)
...
rep={'version':'AINA Character Base v1','status':'body_rig_base','body_vertices':len(body.data.vertices),'body_polygons':len(body.data.polygons),'height_m':b-a,'rig_bones':len(rig.data.bones),'parent_ok':body.parent==rig,'next':'graft AINA v15 head'}
