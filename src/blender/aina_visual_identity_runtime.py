...
def main():
    ...
    # Add actual 3D brows and eyelash curves after the eye-system face is built.
    ...
    def create_face_with_details(face_path,height,skin,eye_mat,teeth_mat,mouth_mat):
        ...
        brow_mat=bpy.data.materials.get('AINA_Hair_Silver') or better_material('AINA_Brow',(.22,.25,.32,1),0,.34)
        def curve(name,pts,radius,mat):
            ob=visual.base.create_curve(name,[tuple(p) for p in pts],radius,mat)
            ob.parent=head; return ob
        # Eyelashes: slim almond upper contour with a short lifted outer tail.
        for side,rr,sign in [('R',range(36,42),-1.0),('L',range(42,48),1.0)]:
            c=lm[list(rr)].mean(0);rx=.0167;rz=.0060
            pts=[(c[0]-rx,-.01225,c[2]+.0002),(c[0]-.008*sign,-.01245,c[2]+.0044),(c[0],-.01255,c[2]+.0056),(c[0]+.008*sign,-.01245,c[2]+.0044),(c[0]+rx,-.01225,c[2]+.0007*sign)]
        ...
        # Brows follow the fitted brow landmarks but are shifted just above the
        # skin surface; this removes the old sculpted-ridge/placeholder look.
        for side,ids in [('R',list(range(17,22))),('L',list(range(22,27)))]:
            pts=[]
            for p in lm[ids]:pts.append((float(p[0]),float(min(p[1]-.0020,-.0105)),float(p[2]+.0018)))
