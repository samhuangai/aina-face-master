...
def main():
    ...
    def w(coords,c,r,inner=0.0,outer=1.0):
        c=np.asarray(c,float);r=np.asarray(r,float);q=np.sqrt(np.sum(((coords-c)/r)**2,axis=1));a=np.zeros(len(coords),float);a[q<=inner]=1.0
        m=(q>inner)&(q<outer)
        if np.any(m):
    ...
    def corrected_polish(mapped,head_ids,eye_groups):
        ...
        # Restrained lower third taper + rounded chin.
        z=out[h,2];t=np.clip((1.570-z)/.068,0,1);p=out[h].copy();p[:,0]*=(1-.075*(t**1.30));out[h]=p
        lm=out[visual.K].copy();chin=lm[8]
        sc(out,h,chin,(.040,.042,.037),(.83,.98,.95),0,1.08);sh(out,h,chin,(.042,.042,.040),(0,-.0012,.0012),0,1.02)
        # Mild upper cranium trim. Hair supplies the final silhouette.
        z=out[h,2];wt=np.clip((z-1.635)/.080,0,1);p=out[h].copy();p[:,0]*=(1-.025*wt);out[h]=p
        # Shallow youthful eye sockets / brow plane.
        ...
        for rr in (range(17,22),range(22,27)):
            c=lm[list(rr)].mean(0);sh(out,h,c,(.040,.032,.025),(0,.0045,.0002),0,1.15)
        sh(out,h,lm[27],(.032,.032,.040),(0,.0035,0),0,1.10)
        # Delicate nose while retaining the already correct vertical proportions.
        lm=out[visual.K].copy();nbase=lm[31:36].mean(0);tip=lm[30]
        sc(out,h,nbase,(.028,.026,.028),(.82,1.0,.96),0,1.15);sh(out,h,nbase,(.030,.030,.032),(0,.0020,.0005),0,1.10);sh(out,h,tip,(.022,.026,.026),(0,.0010,.0003),0,1.05)
        # Correct old profile where lips projected in front of nose.
        lm=out[visual.K].copy();mouth=lm[48:60].mean(0)
        sh(out,h,mouth,(.052,.040,.038),(0,.0070,0),0,1.20);sc(out,h,mouth,(.044,.032,.025),(.96,1.0,.88),0,1.15)
        sh(out,h,lm[51],(.025,.030,.025),(0,.0020,0),0,1.00);sh(out,h,lm[57],(.027,.030,.025),(0,.0015,.0006),0,1.00)
        # Small soft apple-cheek support.
        lm=out[visual.K].copy()
        for c in ((lm[40]+lm[31]+lm[48])/3,(lm[46]+lm[35]+lm[54])/3):sh(out,h,c,(.042,.040,.040),(0,-.0014,.0005),0,1.10)
        # Smaller tucked ears.
        lm=out[visual.K].copy()
        for ii in (0,16):
            c=lm[ii];sc(out,h,c,(.034,.043,.055),(.78,.82,.78),0,1.12);sh(out,h,c,(.034,.043,.055),((.004 if c[0]<0 else -.004),.004,0),0,1.05)
        # Hidden source eye components remain coherent only for topology/debug.
        lm=out[visual.K].copy()
    ...
    def create_face_with_details(face_path,height,skin,eye_mat,teeth_mat,mouth_mat):
        ...
        head,eyes,mapped,groups,head_root,oral_roots,tongue_ids=result;lm=mapped[visual.K]
        lash_mat=bpy.data.materials.get('AINA_Pupil') or better_material('AINA_Lash',(.008,.010,.018,1),0,.22)
        brow_mat=better_material('AINA_Brow',(.18,.20,.26,1),0,.36)
        lip_mat=better_material('AINA_Lip',(.56,.19,.22,1),0,.38);head.data.materials.append(lip_mat);lip_index=len(head.data.materials)-1
        def curve(name,pts,radius,mat):
            ob=visual.base.create_curve(name,[tuple(p) for p in pts],radius,mat);ob.parent=head;return ob
        # Slim real 3D upper lashes and outer tails.
        for side,rr in [('R',range(36,42)),('L',range(42,48))]:
            c=lm[list(rr)].mean(0);rx=.0155
            pts=[(c[0]-rx,-.01185,c[2]+(.0009 if side=='R' else .0002)),(c[0]-rx*.52,-.01205,c[2]+.0037),(c[0],-.01215,c[2]+.0048),(c[0]+rx*.52,-.01205,c[2]+.0037),(c[0]+rx,-.01185,c[2]+(.0002 if side=='R' else .0009))]
            curve(f'AINA_Lash_{side}',pts,.00060,lash_mat);outer=np.array(pts[0 if side=='R' else -1],float)
            curve(f'AINA_LashTail_{side}',[outer,outer+np.array([-.0035 if side=='R' else .0035,-.0001,.0019])],.00048,lash_mat)
        # Brows stay close to fitted landmarks instead of being artificially high.
        for side,ids in [('R',list(range(17,22))),('L',list(range(22,27)))]:
            pts=[(float(p[0]),float(min(p[1]-.0018,-.0103)),float(p[2]+.0004)) for p in lm[ids]];curve(f'AINA_Brow_{side}',pts,.00092,brow_mat)
        # Real material assignment on deforming face polygons gives lips colour
        # without a floating decal and automatically follows all 52 face shapes.
        mc=lm[48:60].mean(0);rx_l=.0235;rz_l=.0083
        for poly in head.data.polygons:
            ...
            if q<1.0 and cen[1]<.005:poly.material_index=lip_index
        return result
