...
Replaces the protruding FaceVerse eye components with smooth almond eye shells,
iris/pupil geometry and expression shape keys. Everything remains real mesh
geometry and VRM morph bindings; no texture-card/reference-image shortcut.
"""
...
def _almond(name, center, material, side):
    c=np.asarray(center,float); rx=.0168; rz=.0061; n=48
    boundary=[]
    # Upper lid: outer -> inner, slightly lifted outside.
    for i in range(n+1):
        u=-1.0+2.0*i/n
        z=rz*(max(0.0,1-u*u)**.58) + (0.00055*u if side=='L' else -0.00055*u)
        y=-.0093 - .0013*(1-u*u)
        boundary.append((c[0]+rx*u,y,c[2]+z))
    # Lower lid: inner -> outer; flatter than upper lid for AINA almond shape.
    for i in range(n+1):
        u=1.0-2.0*i/n
        z=-rz*.70*(max(0.0,1-u*u)**.72) + (0.00030*u if side=='L' else -0.00030*u)
        y=-.0092 - .0010*(1-u*u)
        boundary.append((c[0]+rx*u,y,c[2]+z))
    verts=[(c[0],-.0106,c[2])]+boundary
    faces=[]
    m=len(boundary)
    ...
    for p in kw.data: p.co.z=c[2]+(p.co.z-c[2])*1.16
    ks=ob.shape_key_add(name=squint)
    for p in ks.data: p.co.z=c[2]+(p.co.z-c[2])*.52
    return ob


def _disc(name, center, radius, material, side, pupil=False):
    c=np.asarray(center,float); n=48; verts=[tuple(c)]; faces=[]
    # Slight vertical oval and mild convexity.
    for i in range(n):
        a=2*math.pi*i/n
        x=radius*math.cos(a); z=radius*(1.04 if not pupil else 1.0)*math.sin(a)
        y=c[1]-.00030*(1-(x/max(radius,1e-6))**2)
    ...
    kb=ob.shape_key_add(name=blink)
    for p in kb.data: p.co.y += .030
    # ARKit eye-look controls are actual morphs on iris/pupil geometry.
    dirs={
      ('L','eyeLookUpLeft'):(0,0,.0022),('R','eyeLookUpRight'):(0,0,.0022),
      ('L','eyeLookDownLeft'):(0,0,-.0020),('R','eyeLookDownRight'):(0,0,-.0020),
      ('L','eyeLookInLeft'):(-.0020,0,0),('R','eyeLookInRight'):(.0020,0,0),
      ('L','eyeLookOutLeft'):(.0020,0,0),('R','eyeLookOutRight'):(-.0020,0,0),
    }
...
def _curve(name, pts, radius, material):
    cu=bpy.data.curves.new(name+'_Curve','CURVE'); cu.dimensions='3D'; cu.resolution_u=4; cu.bevel_depth=radius; cu.bevel_resolution=3
...
def install(visual, release):
    ...
    def create_face_objects(face_path:Path,height,skin,eye_mat,teeth_mat,mouth_mat):
        ...
        eyes=[]
        for side in ('R','L'):
            c=centers[side].copy(); c[1]=0.0
            eo=_almond('AINA_Eye_'+side,c,eye_mat,side)
            # Return original eye-component ids only for API compatibility.
            rid=eye_roots[0] if side=='R' else eye_roots[1]
            eyes.append((eo,np.asarray(groups[rid],np.int32),c))
    ...
    def create_uv_sphere(name,location,scale,material,parent=None,rig=None):
        if name.startswith('AINA_Iris_') or name.startswith('AINA_Pupil_'):
            ...
            # Base assembly gives the correct x/z. Use a shallow 3D disc instead of
            # a second sphere so iris/pupil never float in front of the face.
            radius=.0054 if name.startswith('AINA_Iris_') else .00245
            loc[1]=-.01125 if name.startswith('AINA_Iris_') else -.01165
    ...
    def configure_expressions(rig,head):
        ...
        # Add identical expression control to custom eye meshes whenever present.
    ...
    # Expression QA must drive every real mesh carrying that control, not only head.
    original_render=visual.polished_setup_render
    def setup_render(out:Path):
        scene=bpy.context.scene
        # We use visual's camera/light setup by duplicating its core but apply values
        # to all shape-key meshes.
        scene.render.engine='BLENDER_EEVEE_NEXT'; scene.render.image_settings.file_format='PNG'; scene.render.film_transparent=False; scene.world.color=(.055,.065,.085)
        ...
        render('AINA_REAL_FULL_BODY_FRONT.png',(0,-4.7,1.05),(0,0,.98),{},(1024,1536));clear_all()
